...
val desktopAppVersionFile = layout.projectDirectory.file("src/main/resources/app-version.txt")
val desktopPackageVersion = desktopAppVersionFile.asFile.readText().trim().also { version ->
    check(Regex("\\d+\\.\\d+\\.\\d+").matches(version)) {
        "Windows 版本必须是 x.y.z：$version"
    }
}
...
compose.desktop.application {
    ...
    nativeDistributions {
        ...
        windows {
            ...
            msiPackageVersion = desktopPackageVersion
...
val packageReleaseWindowsExe by tasks.registering(Exec::class) {
    ...
    inputs.file(desktopAppVersionFile)
