...
class PosterCache internal constructor(
        ...
        try {
            keyLock.mutex.withLock {
                ...
                try {
                    ...
                    target.toFile().takeIf { it.exists() && it.length() > 0L }
    ...
    suspend fun clearShow(showKey: String) = withContext(Dispatchers.IO) {
        ...
        maintenanceMutex.withLock {
            ...
            // deleteRecursively 抛异常时 remove 不执行, 自增值保留兜底。
    ...
    private fun targetPath(showSegment: String, imageBasename: String, sourceIdentity: String): Path {
        ...
        val extension = rawExtension.lowercase().takeIf { it.matches(Regex("[a-z0-9]{1,8}")) } ?: "img"
    ...
    private fun safeSegment(raw: String): String {
        ...
        val changed = base != raw
        return if (changed) "${base.take(96)}-${sha256(raw).take(12)}" else base.take(112)
    }

    private fun isCompleteFile(path: Path): Boolean =
        Files.isRegularFile(path) && runCatching { Files.size(path) > 0L }.getOrDefault(false)
