...
/**
 * 桌面(JVM/Linux) HttpClient 工厂, 对应 androidMain 的 HttpClientFactory.kt。
 *
 * 与 android 版一致: 用 OkHttp 引擎(JVM 通用), 返回**进程级共享单例**。
 * OkHttp 引擎支持自定义 HTTP 方法(如 WebDAV PROPFIND)。HttpClient(OkHttp) 内部持有
 * 连接池 + Dispatcher 线程池, 重复创建会泄漏线程池。共享单例是 OkHttp 推荐用法。
 *
 * 请求级配置(auth/header/Range)在请求时注入, 不依赖 HttpClient 级配置, 故 WebDAV/
 * 弹弹play/远程哈希可共享同一实例。调用方**不应 close()** 返回的实例。
 *
 * 引擎配置与 android 版逐值一致(单例创建时设; TLS 开关值动态读, 非创建时快照):
...
private val systemTrustManager: X509TrustManager by lazy {
    ...
    // null KeyStore = 加载平台默认信任材料(系统 CA), 等同 OkHttp 默认信任路径。
